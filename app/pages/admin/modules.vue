...
<section>
	<header class="admin-page-heading">
		<div>
			...
			<h1>模块管理</h1>
			<p>控制公开模块的启用状态和展示顺序。提交后先生成预览，不直接修改生产分支。</p>
		</div>
	...
	<div class="module-grid">
		<article v-for="(module, index) in modules" :key="module.id" class="admin-panel module-card">
			...
			<div class="module-order">
				...
				<button class="admin-icon-button" type="button" aria-label="上移模块" :disabled="index === 0" @click="move(index, -1)">
					<Icon name="tabler:arrow-up" />
				</button>
				<button class="admin-icon-button" type="button" aria-label="下移模块" :disabled="index === modules.length - 1" @click="move(index, 1)">
