...
当前财经源每 5 分钟同步最新 125 条作为滚动快照。华尔街见闻单次接口最多返回 100 条，因此同步通过游标分页请求 100 + 25 条；125 条抓取窗口用于覆盖短时上游抖动和高峰期，但 D1 不再每轮整批删除再重写：同步先比较当前快照，只 upsert 新增或内容发生变化的条目，再删除已经退出本轮 125 条窗口的旧条目。若本轮内容完全未变化，只更新来源同步状态，从而降低 D1 写放大和公开缓存无效刷新。
