...
- AI HOT 精选条目 JSON：最短每 15 分钟拉取；
- AI HOT 全文 Feed：最短每 15 分钟拉取，只保存 Feed 明确提供的 `content:encoded`，其余条目保存摘要；
- AI HOT 每日报告 JSON：最短每 15 分钟拉取；
- 在花 RSS：最短每 20 分钟拉取，新增或变化条目尝试提取文章正文，失败时回退 RSS 摘要。

Cloudflare Cron `*/5 * * * *` 每 5 分钟只向 `fly-living-content-sync` Queue 投递 `news-sync` 与 `finance-sync` 任务，不再在 Cron invocation 内直接执行抓取、解析和 D1 写入。Queue consumer 使用单消息批次和单并发执行实际任务，失败消息最多重试 3 次并进入 `fly-living-content-sync-dlq`；各 AI 来源仍根据 `next_sync_at` 执行自己的最短周期。请求保存并发送 ETag/Last-Modified，304 不重新解析或写入正文。后台手动同步仍直接强制检查所有来源。
...
AI 自动来源内容采用 15 天滚动保留：`kind <> 'manual'` 且 `fetched_at` 早于 15 天的卡片及其站内正文会在每日内容维护任务中删除，不区分当前是否 `selected`；手动精选不受此清理线影响。AI 日报和孤立排除记录继续保留 180 天。

### 财经 7×24

当前财经源由 5 分钟 Cron 投递 Queue 任务后同步最新 125 条作为滚动快照。华尔街见闻单次接口最多返回 100 条，因此同步通过游标分页请求 100 + 25 条；125 条抓取窗口用于覆盖短时上游抖动和高峰期，但 D1 不再每轮整批删除再重写：同步先比较当前快照，只 upsert 新增或内容发生变化的条目，再删除已经退出本轮 125 条窗口的旧条目。若本轮内容完全未变化，只更新来源同步状态，从而降低 D1 写放大和公开缓存无效刷新。

财经快讯的后台隐藏状态存放在 `finance_flash_exclusions`，不会因为下一轮同步重新出现；退出当前快照后形成的孤立排除记录保留 180 天后清理。上游同步失败时不裁剪当前快照，继续对外提供最近一次成功数据。

Queue 只传递任务元数据，不传正文；按当前调度每天约产生 581 条消息，正常投递约消耗 1743 次 Queue operations（每条通常对应写入、读取、删除各一次），明显低于 Workers Free 每日 10,000 operations。Free 计划 Queue 消息最多保留 24 小时。

当前默认提供方为华尔街见闻 7×24，因此内容、时间戳、分类和重要性口径不要求与财联社电报一致。若后续需要以财联社为准，应通过获授权的数据服务新增独立 `FinanceFlashAdapter` 并切为 primary，当前华尔街见闻 adapter 可保留为 fallback；不得通过未授权页面抓取模拟财联社正式数据源。
