...
当前财经源每 5 分钟请求一次最新 100 条作为滚动快照。抓取窗口保持 100 条用于覆盖短时上游抖动和高峰期，但 D1 不再每轮整批删除再重写：同步先比较当前快照，只 upsert 新增或内容发生变化的条目，再删除已经退出本轮 100 条窗口的旧条目。若本轮内容完全未变化，只更新来源同步状态，从而降低 D1 写放大和公开缓存无效刷新。
