...
- 天气与播放器按模块条件懒加载，禁用时零天气请求、零 Audio 实例。
...
首页静态 HTML检查：`/api/weather` 不存在、`.music-player` 不存在。天气和音乐运行时通过 `LazyWidgetWeather` / `LazyMusicGlobalPlayer` 条件加载。
...
| C4-AC-22–27 | Pass | 禁用模块零请求、Lazy 组件、图片策略、体积与控制台聚焦测试 |
| C4-AC-28–31 | Pass | 9 files / 44 Nuxt+shared tests；16 files / 86 API tests；21 Edge tests；32 E2E pass / 26 device skips；42 routes；0 link/secret error |
