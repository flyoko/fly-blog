...
describe('market 博客主题终端页面', () => {
	...
	it('uses real finance and market APIs while keeping unavailable states explicit', () => {
		...
		for (const week of ['本周', '上周', '前2周', '前3周'])
			expect(page).toContain(week)
		expect(page).toContain('formatSectorWeekRange')
		expect(page).toContain('交易日')
	...
	it('paginates the full sector universe with a user-selectable page size and supports all approved sort columns', () => {
		...
		for (const key of ['changePct', 'mainNetInflow', 'week:0', 'week:1', 'week:2', 'week:3'])
