...
describe('admin shell contracts', () => {
	...
	it('supports dark mode, liquid glass, and reduced accessibility fallbacks in admin surfaces', async () => {
		...
		expect(stylesheet).toContain('Data-dense admin cards keep the same glass gradients')
		expect(stylesheet).toContain('@media (max-width: 900px), (hover: none) and (pointer: coarse)')
		expect(stylesheet).toContain('.admin-toast,')
		...
		expect(commandPalette).toContain('@media (max-width: 900px), (hover: none) and (pointer: coarse)')
	})
})
