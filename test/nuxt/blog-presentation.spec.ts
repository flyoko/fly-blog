...
describe('public blog presentation', () => {
	...
	it('keeps the desktop shell compact and sticky content below the top navigation', () => {
		...
		const market = read('app/pages/market/index.vue')
		...
		const macWindow = read('app/components/content/MacWindow.global.vue')
		...
		expect(topNav).toContain('grid-template-columns: clamp(13rem, 15vw, 14.5rem)')
		expect(topNav).toContain('width: var(--desktop-top-nav-width')
		expect(topNav).toContain('border-radius: 999px')
		expect(topNav).toMatch(/\.blog-top-nav \{[\s\S]*?backdrop-filter: none;/u)
		expect(topNav).toContain('<BlogHeader class="top-nav-brand top-nav-brand-expanded"')
		expect(topNav).toContain(':scene="!isCondensed"')
		expect(topNav).toContain('const condenseScrollY = 72')
		expect(topNav).toContain('const expandScrollY = 24')
		expect(topNav).toContain('if (scrollY > condenseScrollY)')
		expect(topNav).toContain('\'is-condensed\': isCondensed')
		expect(topNav).toContain('.blog-top-nav:not(.is-condensed)')
		expect(topNav).toContain(':deep(.is-header .scene-character)')
		expect(navBrand).not.toContain('scene?: boolean')
		...
		expect(news).not.toContain('/api/finance/flash')
		expect(news).not.toContain('财经 7×24')
		expect(market).toContain('/api/finance/flash')
		expect(market).toContain('财经 7×24')
		expect(market).toContain('financeImportantOnly')
		...
		expect(main).toContain('.article-window,')
		expect(macWindow).toContain('backdrop-filter: var(--glass-clear-filter)')
		expect(macWindow).toContain(':global(.dynamic #blog-root .admin-app .article-window)')
		...
		expect(main).toMatch(/\.blog-atmosphere \{[\s\S]*?contain: layout style;/u)
		const atmosphereRoot = main.match(/\.blog-atmosphere \{([\s\S]*?)\n\}/u)?.[1] || ''
		expect(atmosphereRoot).not.toContain('will-change: transform')
		expect(atmosphereRoot).not.toContain('transform: translate3d(0, 0, 0)')
	...
	it('composes the about avatar, planet, and character as one coordinated scene', () => {
		...
		expect(page).toContain('gap: var(--mobile-page-gap)')
		expect(page).toContain('width: auto')
		expect(page).toContain('margin: var(--mobile-page-gap) var(--mobile-page-gutter) 1rem')
