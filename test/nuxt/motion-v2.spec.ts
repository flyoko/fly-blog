...
describe('motion v2 production integration', () => {
	...
	it('isolates and temporarily pauses macOS compositor-heavy avatar and atmosphere motion', () => {
		const atmosphere = read('app/components/blog/BlogAtmosphere.vue')
		...
		expect(atmosphere).toContain('const routePointerFailSafeMs = 6_000')
		expect(main).toContain('.blog-atmosphere.is-route-settling')
		expect(main).toContain('animation-play-state: paused')
