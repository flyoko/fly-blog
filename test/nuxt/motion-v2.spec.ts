...
describe('motion v2 production integration', () => {
	...
	it('isolates route-sensitive compositor motion without stopping ambient animation', () => {
		...
		expect(atmosphere).not.toContain('watch(() => route.fullPath')
		...
		expect(atmosphere).not.toContain('nuxtApp.hook(\'page:loading:end\', scheduleRouteResume)')
		expect(atmosphere).not.toContain('nuxtApp.hook(\'page:start\', beginRouteSettling)')
