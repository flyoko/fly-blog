...
describe('motion v2 production integration', () => {
	...
	it('keeps the persistent profile header out of macOS compositor-heavy pointer motion', () => {
		...
		expect(atmosphere).toContain('nuxtApp.hook(\'page:loading:start\'')
		expect(atmosphere).toContain('nuxtApp.hook(\'page:loading:end\'')
		expect(atmosphere).toContain('nuxtApp.hook(\'page:start\'')
		expect(atmosphere).toContain('nuxtApp.hook(\'page:finish\'')
		expect(atmosphere).toContain('Number.POSITIVE_INFINITY')
		expect(atmosphere).toContain('const routePointerSettleMs = 1_200')
		expect(atmosphere).toContain('freezePointerAnimation(performance.now() + routePointerSettleMs)')
